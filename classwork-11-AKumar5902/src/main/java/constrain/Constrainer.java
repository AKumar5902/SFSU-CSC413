...
public class Constrainer extends ASTVisitor {
    private AST t;
    private SimpleScopeManager<AST> scopeManager = new SimpleScopeManager<>();
    ...
    private AST lookup(AST t) {
        return scopeManager.get(((IdTree) t).getSymbol());
    }
    ...
    private void enter(AST t, AST decoration) {
        scopeManager.put(((IdTree) t).getSymbol(), decoration);
    }
    ...
    private void buildIntrinsicTrees() {
        trueTree = new IdTree(Symbol.symbol("true", TokenTypes.Identifier));
        falseTree = new IdTree(Symbol.symbol("false", TokenTypes.Identifier));
        readId = new IdTree(Symbol.symbol("read", TokenTypes.Identifier));
        writeId = new IdTree(Symbol.symbol("write", TokenTypes.Identifier));
        boolTree = (new DeclTree()).addKid(new BoolTypeTree()).
                addKid(new IdTree(Symbol.symbol("<<bool>>", TokenTypes.Identifier)));
        decorate(boolTree.getKid(2), boolTree);
        intTree = (new DeclTree()).addKid(new IntTypeTree()).
                addKid(new IdTree(Symbol.symbol("<<int>>", TokenTypes.Identifier)));
        ...
        AST decl = (new DeclTree()).addKid(new IntTypeTree()).
                addKid(new IdTree(Symbol.symbol("dummyFormal", TokenTypes.Identifier)));
    ...
    public Object visitFunctionDeclTree(AST t) {
        ...
        scopeManager.beginScope();  // new scope for formals and body
    ...
    public Object visitRelOpTree(AST t) {
        AST leftOp = t.getKid(1),
                rightOp = t.getKid(2);
        if (leftOp.accept(this) != rightOp.accept(this)) {
